...
def error_response(status_code, message=''):
    """
    :param message: if None, empty response;
                    if '', response without a message
    """
    if message is None:
        response = ''
    else:
...
def err_network_read_timeout(message=''):
    return error_response(598, message)
